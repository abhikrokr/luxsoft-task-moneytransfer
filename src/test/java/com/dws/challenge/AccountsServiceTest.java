  ...
  void concurrentTransferMoneyTestDifferentAccount() {
    ...
    for(int i = 0;i < 1000000;i++){
      e.submit(new Runnable() {
        ...
        public void run() {
          String from = UUID.randomUUID().toString();
          Account fromAccount = new Account(from,new BigDecimal(10000000));
          ...
          BigDecimal amount = new BigDecimal(ran.nextInt(1000)+1);
          MoneyTransfer mo = new MoneyTransfer(from,too,amount);
          accountsService.transferMoney(mo);

          Account pay = accountsService.getAccount(from);
          Account rei = accountsService.getAccount(too);
          assertThat(pay.getBalance().add(rei.getBalance())).isEqualByComparingTo(new BigDecimal(10000000));
        }
      });
    }
  }
  ...
  void concurrentTransferMoneyTestSameAccount() {
    ...
    for(int i = 0;i < 1000000;i++){
        ...
        e.submit(() -> {
          BigDecimal amount = new BigDecimal(ran.nextInt(1000)+1);
          if(ran.nextBoolean()){
            accountsService.transferMoney(new MoneyTransfer(from,too,amount));
          }else{
            accountsService.transferMoney(new MoneyTransfer(too,from,amount));
          }
        });
    }
    Account pay = accountsService.getAccount(from);
    ...
    assertThat(pay.getBalance().add(rei.getBalance())).isEqualByComparingTo(new BigDecimal(2000000));
  }
